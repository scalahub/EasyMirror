...
object Util extends TraitFilePropertyReader {
  ...
  val defaultPrefix =
    "_" // Methods starting with this will be considered in the EasyProxy
  val defaultMetaPrefix =
    "____" // Meta methods start with this. Meta methods are methods about methods (i.e., giving info about other methods)
  def getDefaultPid(cls: AnyRef) = {
    ...
    org.sh.utils.Util.sha256Small(
      org.sh.utils.Util.sha256Small(
        cls.getClass.getCanonicalName
      ) + "iowy8uhciergf674tgycb4yt4gb"
    ) collect {
      case c if "ABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789".contains(c.toUpper) => c
    }
  }
  def NOTImplementedOut[T <: AnyRef](t: T): String =
    throw new Exception(t.getClass.getName + ": Output type not implemented")
  def NOTImplementedIn[T <: AnyRef](s: String): T =
    throw new Exception(s + ": Input type not implemented")
  ...
  def getMethods(c: AnyRef, is: java.io.InputStream) = {
    ...
    if (methods.size > 0) for (i <- 1 to methods.size) {
      ...
      try {
        ...
        val vars =
          m.localVariables.asInstanceOf[java.util.List[LocalVariableNode]];

        var paramList: List[Param] = Nil
        if (argTypes.length > 0 && vars.asScala.length > 0) {
          val methodVars = vars.asScala
            .map { v => v.index -> MethodVar(v.name, v.desc) }
            .sortBy(_._1)
        ...
        val infoVars: Option[LocalVariableNode] =
          vars.asScala
            .find(_.name == "$INFO$")
            .find(info =>
              Type.getType(info.desc) == Type.getType(classOf[String])
            )

        // val with name "$INFO$" and type String gives info about method
        // group seems to be unused
        val groupVars = vars.asScala
          .find(_.name == "$group$")
          .find(groupDesc =>
            Type.getType(groupDesc.desc) == Type.getType(classOf[String])
          ) // var names "$group$" gives info about which group method belongs

        val allVars = vars.asScala
          .filter(x => x.name.startsWith("$") && x.name.endsWith("$"))
          .filter { v =>
        ...
        def getStr(optVarNode: Option[LocalVariableNode]) = {
          ...
          optVarNode.map { varNode =>
            try {
              val trialNode = varNode.start.getPrevious
              val (opCodeTry1, typeTry1) =
                (trialNode.getOpcode, trialNode.getType)
              val actualNode =
                if ((opCodeTry1, typeTry1) == (18, 9)) trialNode
                else trialNode.getPrevious

              val textNode = actualNode
        ...
        val allStr = allVars
          .map(allVar => allVar.name -> getStr(Some(allVar)))
          .collect {
            case (name, Some(value)) => name -> value
          }
          .toMap

        val paramVars = paramList
          .map { p =>
            vars.asScala
              .find(_.name == "$" + p.paraName + "$")
              .find(info =>
                Type.getType(info.desc) == Type.getType(classOf[String])
              )
          }
          .map(getStr)
        (paramList zip paramVars) foreach {
          case (p, optInfo) =>
            p.info = optInfo
        }
        val sm = ScalaMethod(
        ...
        case e: Throwable =>
          println(
            s" [ERROR] Method [${m.name}]. Error message: " + e.getMessage
          )
        // e.printStackTrace()
      }
    }
  ...
  def getMethods(c: Class[_], processSuperClass: Boolean): List[ScalaMethod] = {
    ...
    val is =
      try {
        ...
        if (is == null)
          throw new IllegalArgumentException("Cannot find class: " + url);
        else is
      } catch {
        case e: IllegalArgumentException =>
          println(
            " [IllegalArgumentException] " + Thread
              .currentThread()
      ...
      if (processSuperClass) {
        ...
        (
          superClass.getName match {
            ...
            case _ =>
              getMethods(superClass, processSuperClass: Boolean).filterNot {
                m =>
                  thisClassMethodsSig.contains(
                    m.canonicalSignature
                  ) // do not consider overridden methods
              }
          }
        )
      } else Nil
  ...
  def getJavaMethod(c: AnyRef, m: ScalaMethod): Option[Method] = {
    val x = c.getClass.getMethods.find(x =>
      x.getName == m.name && paramsMatch(m.params, x.getParameterTypes)
    )
    x
  }
