...
class TypeHandler {
  ...
  def initialize = {
    ...
    def strToBigInt(string: String): BigInt = {
      if (string.startsWith("0x")) BigInt(string.drop(2), 16) else BigInt(string, 10)
    }

    // add more complex types
    addType[String](classOf[String], identity, identity)
    addType[BigInt](classOf[BigInt], strToBigInt, bigInt => s""""${bigInt.toString(10)}"""")
    ...
    addType[Array[Array[Byte]]](
      ...
      arrArrByte => {
        val arrEncoded = arrArrByte.map { arrByte =>
          Hex.encodeBytes(arrByte)
        }
