...
object ProxyTest extends App{
  ...
  assert(
    ...
    """["def F8(a:int, b:String): int","def F1(a:int): boolean","def F4(a:String, b:String, c:String[], d:mux.reflect.test.D): scala.Tuple2[]","def F5(a:int, s:String, d:mux.reflect.test.D): mux.reflect.test.D[]","def F6(a:mux.reflect.test.D, b:mux.reflect.test.D, c:mux.reflect.test.D, s:String): mux.reflect.test.D","def F3(a:int[]): String[]","def F2(a:String): String","def F7(a:int): String"]"""
  )
  ...
  assert(Proxy.getResponse(metaPid(myObjectPID), "getMethodInJava", "{'name':'F6'}") == """["mux.reflect.test.D F6(mux.reflect.test.D a,mux.reflect.test.D b,mux.reflect.test.D c,String s)"]""")
  assert(Proxy.getResponse(metaPid(myObjectPID), "getMethodInScala", "{'name':'F6'}") == """["def F6(a:mux.reflect.test.D, b:mux.reflect.test.D, c:mux.reflect.test.D, s:String): mux.reflect.test.D"]""")
