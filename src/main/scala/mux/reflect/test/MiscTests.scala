...
object FormProcessorTest extends App {
  val m = mux.reflect.Util.getMethods(mux.reflect.test.MyObject, true)
  m.foreach(x => mux.reflect.Util.printMethod(x))
}

object RunMethodSpyJava {
  def main(arg:Array[String]) = MethodSpy.main("mux.reflect.test.MethodSpy", "m1")
}
