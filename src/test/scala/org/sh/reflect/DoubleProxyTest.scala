...
object TestDoubleProxyServer extends App {
  ...
  assert(
    EasyProxy
      .getResponse("myObjectID", "otherMethod", "{'myParam':'hello'}") == "Ok"
  )
...
// The implementations are available in ReflectSocket and ReflectWeb, where ProxyQueryMaker can be tested
